		...
		if (ptXMI!= null) {
			...
			for (int i=0;i<xy.length;i++) {
				...
				if (p != null && p.length == 2) {
					int x = Integer.valueOf(p[0].trim());
					int y = Integer.valueOf(p[1].trim());
	...
	public Rectangle computeRectID2(String rectXMI) {
		if (rectXMI!= null) {
			...
			if (xywh != null && xywh.length == 4) {
				x = Integer.valueOf(xywh[0].trim());
				y = Integer.valueOf(xywh[1].trim());
				w = Integer.valueOf(xywh[2].trim());
				h = Integer.valueOf(xywh[3].trim());
			}
